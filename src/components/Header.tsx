    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200 bg-white shadow-sm">
      <div className="mx-auto flex h-20 w-full max-w-[1400px] items-center justify-between gap-3 px-4 lg:px-6">
        <Link to="/" className="flex shrink-0 items-center gap-3" aria-label="Rentmeister home">
          <img src="/images/logo.png" alt="Rentmeister logo" className="h-14 w-14 rounded-full object-contain" />
          <div className="hidden sm:block">
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#1a2744]">Rentmeister</p>
            <p className="text-[11px] font-medium uppercase tracking-[0.1em] text-slate-500">Total Home Service</p>
          </div>
        </Link>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex-1 rounded-lg px-3 py-2 text-sm font-medium transition ${
                        isActive ? 'bg-[#c0392b]/10 text-[#c0392b]' : 'text-[#1a2744] hover:bg-slate-50'
                      }`
                    }
                  >
                    {item.label}
                  ...
                  <div className="mt-1 space-y-1 border-l border-slate-200 pl-2">
                    {item.submenu.map((child) => (
                      <Link
                        ...
                        to={child.to}
                        onClick={() => setIsOpen(false)}
                        className="block rounded-lg px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-[#1a2744]"
                      >
                        {child.label}
                      </Link>
                    ))}
